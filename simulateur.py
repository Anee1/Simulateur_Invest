...
if st.button("🚀 Lancer la simulation", use_container_width=True):
    ...
    valeurs_dat, valeurs_E_dat = simulation(montant_initial, duree_investissement, 0.035, montant_periodique, frequence, annees_contributions)
    ...
    resultats = pd.DataFrame(
        ...
        index=[f"{fond_choisi}", "DAT 3.5%", f"Écart ({fond_choisi} - DAT)","Montant Épargné"],
        columns=colonnes
    )
